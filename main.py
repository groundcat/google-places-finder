...
with open(f'{QUERY} in {LOCATION} ({PAGES} pages) {timestamp}.csv', 'w') as f:
    writer = csv.writer(f)
    writer.writerow(
        ['name', 'geo_location', 'place_id', 'details', 'local_phone_number', 'international_phone_number', 'website',
         'url'])
    for place in result_list:
        writer.writerow(
            [place['name'], place['geo_location'], place['place_id'], place['details'], place['local_phone_number'],
